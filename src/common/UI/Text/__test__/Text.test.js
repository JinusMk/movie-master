...
describe('<Input />', () => {
  ...
  it('should render Text with extra large 4 font-size', async () => {
    render(<Text fontSize={'xl-4'}>My text</Text>)

    expect(screen.getByText('My text')).toHaveClass('xl-4', {
      exact: false,
    })
  })

  it('should render Text with extra large 5 font-size', async () => {
    render(<Text fontSize={'xl-5'}>My text</Text>)

    expect(screen.getByText('My text')).toHaveClass('xl-5', {
      exact: false,
    })
  })

  it('should render Text with extra large 6 font-size', async () => {
    render(<Text fontSize={'xl-6'}>My text</Text>)

    expect(screen.getByText('My text')).toHaveClass('xl-6', {
      exact: false,
    })
  })
})
